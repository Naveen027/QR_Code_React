...
function App() {
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      number,
      table_id: tableId,
      items: cart.map(i => ({ item: i.name, qty: i.qty }))
    };

    try {
      const res = await fetch("https://qr-code-flask-3zbw.onrender.com/submit-order", {
        method: "POST",
    ...
    <div className="app-container">
      ...
      {orderSummary && (
        <div className="order-summary" ref={summaryRef}>
          ...
          <p><strong>Total:Rs:</strong> {orderSummary.total}</p>
